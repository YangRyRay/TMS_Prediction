...
def dotproduct(v1, v2):
    v1_ten = v1.cpu().detach().numpy()
    v2_ten = v2.cpu().detach().numpy()
    return sum((a*b) for a, b in zip(v1_ten, v2_ten))

def length(v):
    a = dotproduct(v, v)
    return np.sqrt(a)

def loss_gradient_difference(generated, real_image): # b x c x h x w  #(3, 120, 220, 180) C DHW
    print(real_image.shape) # (2, 3, 120, 220, 180)
    ###  x  ###
    ...
    loss_x_gradient = torch.mean(difference_x) #(torch.sum(difference_x)**2)/2 # tf.nn.l2_loss(true_x_gradient - generated_x_gradient)
    ...
    loss_y_gradient = torch.mean(difference_y) #(torch.sum(difference_y)**2)/2 # tf.nn.l2_loss(true_y_gradient - generated_y_gradient)
    ...
    loss_z_gradient = torch.mean(difference_z) #(torch.sum(difference_z)**2)/2 # tf.nn.l2_loss(true_y_gradient - generated_y_gradient)
    print(loss_x_gradient, loss_y_gradient, loss_z_gradient)

    igdl = loss_x_gradient + loss_y_gradient + loss_z_gradient
    return igdl


class MSELoss_Angleloss(nn.Module):
    
    def __init__(self, alpha, beta, gamma):
        super(MSELoss_Angleloss, self).__init__()
        self.alpha = alpha
        self.bce = nn.BCEWithLogitsLoss()
        self.beta = beta
        self.gamma = gamma
    ...
    def forward(self, input, target): ## target is the gt, input is the prediction
        
        # mask #
        #print(real_image.shape) # (2, 3, 120, 220, 180)
        mask_top = np.ones([target.size()[0], target.size()[1], target.size()[2], target.size()[3], target.size()[4]])
        top_value_np = np.percentile(target.cpu().numpy(), [99.75])
        print("top_value:", top_value_np)
        mask_top[(target.cpu().numpy() > top_value_np)] = 1.2
        mask_top = torch.from_numpy(mask_top)
        mask_top = mask_top.to(device=torch.device('cuda:0'))
        target_np = target.cpu().numpy()
        _range = np.max(target_np) - top_value_np
        gt_data_norm = (target_np - top_value_np) / _range
        weight_np = 2 - gt_data_norm
        weight_np[(target_np <= top_value_np)] = 1
        weight = torch.from_numpy(weight_np)
        weight = weight.to(device=torch.device('cuda:0'))
        #print(torch.sum(mask_top == 1.2)) # 71280
        
        #print(length(input))
        angle = dotproduct(input, target) - (length(input) * length(target)) * weight_np
        angleloss = torch.abs(torch.mean(torch.Tensor(angle)))
        
        mse_metric = (target-input)**2
        mse_norm = F.normalize(mse_metric, p=2, dim=1)
        mseloss = mse_metric # * weight
        print("mse_metric max:", mse_metric.max(),"mse_metric mean:", mseloss.mean())
        
        ## gradient loss ##
        gradloss = loss_gradient_difference(input, target)
        
        print("angleloss:", angleloss, "mseloss:", self.mse(input, target), "gradloss:", gradloss)
        return self.alpha * mseloss.mean() + self.beta * angleloss + self.gamma * gradloss  #self.mse(input, target)
...
def _create_loss(name, loss_config, weight, ignore_index, pos_weight):
    ...
    elif name == 'MSELoss_Angleloss':
        return MSELoss_Angleloss(1, 1, 1)
    else:
