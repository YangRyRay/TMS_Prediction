...
from Lib.pytorch3dunet.datasets.hdf5 import AbstractHDF5Dataset
from Lib.pytorch3dunet.datasets.utils import SliceBuilder
from Lib.pytorch3dunet.unet3d.utils import get_logger
from Lib.pytorch3dunet.unet3d.utils import remove_halo
...
class H5lessPredictor(_AbstractPredictor):
    """
    Modified StandardPredictor trying to remove H5 file output and just return a matrix
    ...
    def __call__(self, test_loader):
        ...
        # save results
        result = self._save_results(prediction_maps, normalization_masks, output_heads, test_loader.dataset)
        # close the output H5 file
        # h5_output_file.close()
    ...
    def _save_results(self, prediction_maps, normalization_masks, output_heads, dataset):
        def _slice_from_pad(pad):
            if pad == 0:
                return slice(None, None)
            else:
