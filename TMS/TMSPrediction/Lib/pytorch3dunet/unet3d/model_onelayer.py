...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        self.final_conv_theta = nn.Conv3d(f_maps[0], 1, 1)
        self.final_conv = nn.Conv3d(1, 3, 1)
        self.phi_conv = nn.Conv3d(3, 3, 1)
        if is_segmentation:
    ...
    def forward(self, x):
        ...
        x = self.final_conv_theta(x)
        #print("x shape:", x.shape) #[2, 1, 120, 220, 180]
        #x = torch.cat([x,x,x],dim=1) #[2, 3, 120, 220, 180]
        x = self.final_conv(x)
        input_x = self.phi_conv(input_x[:,:3,:,:,:])
        x = x + input_x
