...
def _create_trainer(config, model, optimizer, lr_scheduler, loss_criterion, eval_criterion):
...
class UNet3DTrainer:
    ...
    def __init__(self, model, optimizer, lr_scheduler, loss_criterion,
                 eval_criterion, device, checkpoint_dir,
    ...
    @classmethod
    def from_checkpoint(cls, resume, model, optimizer, lr_scheduler, loss_criterion, eval_criterion,
        ...
        return cls(model, optimizer, lr_scheduler,
                   ...
                   torch.device("cuda:0"),#state['device']),
                   checkpoint_dir,
    ...
    def from_pretrained(cls, pre_trained, model, optimizer, lr_scheduler, loss_criterion, eval_criterion,
        ...
        return cls(model, optimizer, lr_scheduler,
                   loss_criterion, eval_criterion,
                   torch.device("cuda:0"), checkpoint_dir,
    ...
    def train(self):
        ...
        ### wyx data ###
        for i in range(24):
            self.loaders = get_train_loaders(self.config, i)
            ...
            for t in self.loaders['train']:
                ...
                #print(input.max(), input.mean(), target.max(), target.mean(), output.max(), output.mean())
    ...
    def gradient_map(self,real_image, generated): # b x c x h x w  #(3, 120, 220, 180) C DHW
        ...
        true_gradient_map = torch.sqrt(1e-8 + (true_x_gradient[:,:,:-1,:,:-1])**2 + (true_y_gradient[:,:,:-1,:-1,:])**2 + (true_z_gradient[:,:,:,:-1,:-1])**2)
        generated_gradient_map = torch.sqrt(1e-8 + (generated_x_griednt[:,:,:-1,:,:-1])**2 + (generated_y_griednt[:,:,:-1,:-1,:])**2 + (generated_z_griednt[:,:,:,:-1,:-1])**2)
        difference_map = torch.abs(true_gradient_map - generated_gradient_map)
        
        return true_gradient_map, generated_gradient_map, difference_map

    def _log_images(self, input, target, prediction, prefix=''):
        true_gradient_map, generated_gradient_map, difference_map = self.gradient_map(target, prediction)
        inputs_map = {
            'inputs': input,
            'targets': target,
            'predictions': prediction,
            'true_gradient_map': true_gradient_map,
            'generated_gradient_map': generated_gradient_map,
